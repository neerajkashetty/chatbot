...
const Home = () => {
  ...
  const delayedChatbotResponse = () => {
    setTimeout(() => {
      ...
      setChatLog((prevChatLog) => [
        ...prevChatLog,
        { type: 'chatbot', message: "I can't answer anything right now" }
      ]);
    }, 3000);
    setIsLoading(false)
  };
  ...
  return (
    <div className='app bg-zinc-800 flex'>
      <section className='side-bar bg-zinc-900 h-lvh w-1/5 flex flex-col justify-start shadow-lg'>
        <button className='border-solid border-2 border-slate-400 bg-transparent rounded-md p-3 m-3 text-white focus:outline-none hover:bg-green-800 hover:border-none transition-colors duration-300'>+ New Chat</button>
      ...
      <section className='main w-2/3 flex flex-col justify-end items-center shadow-md overflow-auto'>
        <div className='chat-section w-full flex flex-col justify-end items-center'>
          {/* <div className='flex flex-col items-end flex-grow overflow-auto p-6'>
                ...
                chatLog.map((message, index) => {
                  return (
                    <div key={index} className={`flex ${
                      ...
                      <div className= { `${message.type === 'user' ? 'bg-green-900/[.7] rounded-r-lg self-end' : 'bg-gray-800 rounded-r-lg self-start'} rounded-lg p-4 text-white max-w-xs md:max-w-full`} >
                        {message.message}
                      </div>
                    </div>
                  )
          ...
          <div className='chat-section w-full p-6 flex flex-col items-start space-y-4'>
            {chatLog.map((message, index) => (
                <div key={index} className={`chat ${message.type === 'user' ? 'chat-start' : 'chat-end'}`}>
                  ...
                  <div className={`chat-bubble ${message.type === 'user' ? 'bg-green-900/[.7]' : 'bg-gray-800'}`}>
                    {message.message}
                  </div>
                </div>
              ))}
          </div>
          <div className='input-container my-4 w-full flex max-w-2xl justify-center relative'>
            <input type="text" className='flex-grow bg-white/[.05] border-none py-3 px-3 rounded-md outline-none shadow-md text-white' placeholder='Type your message...' 
              value={searchTerm}
              onChange={handleChange}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
            ...
            />
            <button type="submit" className='bg-green-900/[.7] rounded-md px-4 py-3 text-white font-semibold shadow-md focus:outline-none hover:bg-green-800 transition-colors duration-300' onClick={handleSend}>Send</button>
          </div>
        </div>
      </section>
